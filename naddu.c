...
static inline void gen_pawns_white(Node * __restrict node) {
  ...
  while (quiet) {
    int to = bsf(quiet); quiet &= quiet - 1;
...
static inline void gen_pawns_black(Node * __restrict node) {
...
static inline void gen_pawns(Node * __restrict node) {

  if (node->pos.stm == WHITE)
    gen_pawns_white(node);

  else
    gen_pawns_black(node);
...
static void gen_knights(Node * __restrict node) {

  const Position *pos = &node->pos;
  const int stm = pos->stm;
  const uint64_t friends = pos->colour[stm];
  const uint64_t opp_king = pos->all[piece_index(KING, stm ^ 1)];
  const uint64_t targets = ~friends & ~opp_king;

  uint32_t *m = node->moves + node->num_moves;
  int n = 0;

  uint64_t bb = pos->all[piece_index(KNIGHT, stm)];

  while (bb) {

    const int from = bsf(bb); bb &= bb - 1;
    uint64_t attacks = knight_attacks[from] & targets;
...
static void gen_sliders(Node * __restrict node, Attack * __restrict attack_table, const int piece) {
...
static void gen_queens(Node * __restrict node) {
  ...
  uint32_t *m = node->moves + node->num_moves;
  int n = 0;
  uint64_t q = pos->all[piece_index(QUEEN, stm)];

  while (q) {

    const int from = bsf(q); q &= q - 1;

    const Attack *rb = &rook_attacks[from];
    const Attack *bb = &bishop_attacks[from];

    uint64_t a = rb->attacks[magic_index(occ & rb->mask, rb->magic, rb->shift)] |
                 bb->attacks[magic_index(occ & bb->mask, bb->magic, bb->shift)];
    a &= targets;
...
static inline void gen_kings(Node * __restrict node) {
  ...
  const int opp_king_sq = bsf(pos->all[piece_index(KING, opp)]);
  const uint64_t opp_king_bb = 1ULL << opp_king_sq;
  const uint64_t opp_king_near = king_attacks[opp_king_sq];

  uint32_t *m = node->moves + node->num_moves;
  int n = 0;

  const int from = bsf(pos->all[piece_index(KING, stm)]);
  uint64_t attacks = king_attacks[from];

  attacks &= ~friends;
  attacks &= ~opp_king_bb;
  attacks &= ~opp_king_near;
...
static void gen_moves(Node * __restrict node) {
...
static uint64_t perft(int ply, int depth) {
  ...
  this_node->in_check = is_attacked(this_pos, bsf(this_pos->all[stm_king]), opp);
  gen_moves(this_node);

  for (int i=0; i < this_node->num_moves; i++) {

    *next_pos = *this_pos;                     // copy
    make_move(next_pos, this_node->moves[i]);  // make
...
static int qsearch(int ply, int alpha, int beta) {
  ...
  this_node->in_check = 1; //hack;
  gen_moves(this_node);

  for (int i=0; i < this_node->num_moves; i++) {

    const uint32_t move = this_node->moves[i];
    const int to = (int)(move & 0x3F); //hack do staged movegen
    if (this_pos->board[to] == EMPTY && !(move & FLAG_EP_CAPTURE))
      continue;
...
static int search(int ply, int depth, int alpha, int beta) {
  ...
  if (depth <= 0 && this_node->in_check == 0) {
...
static int uci_tokens(int num_tokens, char **tokens) {
  ...
  if (!strcmp(cmd, "isready")) {
    ...
    if (!move_time && stm == WHITE && wtime)
      move_time = MAX(1, move_time / moves_to_go + winc);
    else if (!move_time && stm == BLACK && btime)
      move_time = MAX(1, move_time / moves_to_go + binc);
    ...
    printf("time %" PRIu64 "ms, nps %" PRIu64 "\n", elapsed_ms, nps);
